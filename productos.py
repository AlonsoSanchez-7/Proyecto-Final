...
from productos import crud_produ
...
def agregarProductos(conexionBD):
    ...
    if crud_produ.insertar(nombre, costo, stock, proveedor, conexionBD):
        accionExitosa()
    else:
        accionNoExitosa()
...
def mostrarProductos(conexionBD):
    ...
    print("="*60)
    registros = crud_produ.consultar(conexionBD)
    
    if registros:
        for r in registros:
            c = float(r[2])
            s = int(r[3])
            m = crud_produ.calcular_metricas(c, s)
        ...
        print(Fore.YELLOW + "="*60)
        resumen = crud_produ.obtener_resumen_global(conexionBD)
        if resumen:
...
def buscarProductos(conexionBD):
    ...
    registros = crud_produ.buscar(nombre, conexionBD)
    
    if registros:
        for r in registros:
            c = float(r[2])
            s = int(r[3])
            dict_m = crud_produ.obtener_metricas_diccionario(c, s)
            print(Fore.YELLOW + "="*60)
            print(Fore.WHITE + Style.BRIGHT + f"  🆔 ID: {r[0]} | 🛠️ Producto: {r[1]} | 🏭 Proveedor: {r[4]}")
...
def modificarProductos(conexionBD):
    ...
    encontrados = crud_produ.buscar(nombre_buscar, conexionBD)
    ...
    if encontrados:
        ...
        if crud_produ.modificar(nombre_nuevo, costo_nuevo, stock_nuevo, proveedor_nuevo, nombre_buscar, conexionBD):
            accionExitosa()
        else:
            accionNoExitosa()
    else:
...
def limpiarProductos(conexionBD):
    ...
    if confirmar == "S":
        if crud_produ.vaciar(conexionBD):
            accionExitosa()
        else:
            accionNoExitosa()
    else:
